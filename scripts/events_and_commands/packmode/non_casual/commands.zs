...
events.onCommand(function (event as CommandEvent) {
    ...
    if (event.commandSender instanceof IPlayer) {
        ...
        val parameters = event.parameters; // 缓存参数数组
        val playerName = player.name;
        val param1 = parameters[1];
        val param2 = parameters[2];

        if ((playerName != "TCreopargh") && ((!isNull(param2) && (param2 == "iswuss" || param2 == "truehero")) || (!isNull(param1) && param1 == "all"))) {
            event.cancel();
